...
def mask2one_hot(label, num_classes):
    current_label = label.squeeze(0)  # （batch_size, 1, h, w) ---> （batch_size, h, w)
    h, w, t =  current_label.shape[0], current_label.shape[1], \
                          current_label.shape[2]

    one_hots = []
    for i in range(num_classes):
        tmplate = torch.ones(h, w, t)  # （batch_size, h, w)
        tmplate[current_label != i] = 0
        tmplate = tmplate.view(1, h, w, t)  # （batch_size, h, w) --> （batch_size, 1, h, w)
...
class NifitDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        label_np = label_np[:, :, :]
        label_np = np.identity(4)[np.uint8(label_np)]
        label_np = np.transpose(label_np, (3, 0, 1, 2))
