...
from utils import AvgMeter, dice_coeff,dice_coeff1, check_dir, DiceLoss
...
def test_epoch(net, loader):
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        test_dice_meter.update(dice_coeff1(output, label))
...
def main(args):
    ...
    if args.do_you_wanna_train is True:
        ...
        train_loader = DataLoader(train_set, batch_size=args.batch_size,num_workers=8, shuffle=True)                                   # Here are then fed to the network with a defined batch size
        test_loader = DataLoader(test_set, batch_size=args.batch_size,num_workers=8, shuffle=False)
        ...
        if args.multi_gpu is True:
            ...
            net = torch.nn.DataParallel((UNet(1,4)).cuda())  # load the network Unet
        else:
            torch.cuda.set_device(args.gpu_id)
            net = UNet(1,4).cuda()

        if args.do_you_wanna_load_weights is True:
            net.load_state_dict(torch.load(args.load_path),False)                                    # load the weights of the network if you have it

        # define the optimizer of the training process                                         # define the optimizer of the training process
        optimizer = torch.optim.Adam(net.parameters(), lr=args.lr, weight_decay=args.weight_decay)

        # define the loss function
        cost = DiceLoss()                                                              # define the loss function
        best_dice = 0.
        for epoch in range(args.init_epoch, args.init_epoch+args.num_epoch):                   # define the epochs number
            ...
            info_line = "Epoch {} || Loss: {:.4f} | Time(min): {:.2f} | | Testing Dice: {:.4f}".format(
                str(epoch).zfill(3), epoch_loss, epoch_time/60,  epoch_dice_test
            )
            print(info_line)
            ...
            if epoch_dice_test > best_dice:
                best_dice = epoch_dice_test
    ...
    if args.do_you_wanna_check_accuracy is True:
        ...
        print("Dice_test:",Dice_test,"Dice_train:",Dice_train)
